...
Phase 4 — Innovation Opportunities:
  ...
  InnovationSaveRequest - input for POST /api/v1/innovation-saves
  SavedInnovationSummary - output: saved-record metadata
  SavedInnovationDetail - output: saved-record metadata + full InnovationResponse payload


Management Summary:
  ManagementSummaryRequest — input for POST /api/v1/management-summaries: the raw Risk/Design/Innovation result payloads
  SavedManagementSummary   — output: saved-record metadata (no PDF bytes — those are fetched separately via the /pdf download route)
